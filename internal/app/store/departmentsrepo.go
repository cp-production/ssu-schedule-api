...
func (r *DepartmentsRepo) Insert(d *model.Departments) error {
	...
	if _, err := r.store.db.Exec(query, d.Url, d.FullName, d.ShortName); err != nil {
		return err
	}
...
func (r *DepartmentsRepo) SelectAll() (*[]model.Departments, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&dep.Url, &dep.FullName, &dep.ShortName); err != nil {
			return nil, err
		}
...
func (r *DepartmentsRepo) SelectById(query_id string) (*model.Departments, error){
	...
	dep := &model.Departments{}
    if err := row.Scan(&dep.Url, &dep.FullName, &dep.ShortName); err != nil {
        return nil, err
    }
    return dep, nil
